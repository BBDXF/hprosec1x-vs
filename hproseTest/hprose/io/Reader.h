...
struct CastError : std::runtime_error {
    explicit CastError(char tag)
        : std::runtime_error(interact::TagToString(tag) + " can't change to " + typeid(T).name()) {
    }
};
...
public:
    Reader(std::istream &stream, bool simple = false)
        : RawReader(stream), refer(simple ? nullptr : new interact::ReaderRefer()) {
    }
...
private:
    std::unique_ptr<interact::ReaderRefer> refer;
};
